...
const Sample = React.forwardRef<HTMLDivElement, Props>(function Sample(
  ...
  return (
    ...
    >
      <div
        ref={scrollRef}
        className="h-full overflow-y-auto hide-scrollbar cursor-pointer"
      >
      ...
      {hasAbove && (
        <div className="pointer-events-none absolute top-0 left-0 right-0 h-14 bg-gradient-to-b from-black/25 to-transparent rounded-t-xl" />
      )}
      {hasMore && (
        <div className="pointer-events-none absolute bottom-0 left-0 right-0 h-14 bg-gradient-to-t from-black/25 to-transparent rounded-b-xl" />
      )}
      {hasMore && (
        <button
          type="button"
